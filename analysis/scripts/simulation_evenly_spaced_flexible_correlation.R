...
# FLEXIBLE CORRELATION STRUCTURES: AR1, Exponential, Power Law, Matérn, Rational Quadratic
...
# ============================================================================
# Choose correlation structure: "ar1", "exponential", "power_law", "matern", "rational_quadratic"

correlation_structure <- "exponential"  # <-- CHANGE THIS TO SELECT STRUCTURE

# Correlation-specific parameters
correlation_params <- switch(correlation_structure,
  ar1 = list(
    rho = 0.75,  # Geometric decay rate
    description = "AR(1): Geometric decay ρ^(time_gap)"
  ),
  exponential = list(
    lambda = 0.10,  # Decay rate (higher = faster decay)
    description = "Exponential/OU: exp(-λ * time_gap)"
  ),
  power_law = list(
    alpha = 0.1,  # Inverse power coefficient
    beta = 1.0,   # Power exponent
    description = "Power Law: 1 / (1 + α * time_gap)^β"
  ),
  matern = list(
    nu = 1.5,     # Smoothness parameter (0.5, 1.5, 2.5)
    rho = 5.0,    # Length scale
    description = "Matérn: Flexible with smoothness parameter ν"
  ),
  rational_quadratic = list(
    alpha = 1.0,  # Smoothness
    lambda = 2.0, # Length scale
    description = "Rational Quadratic: (1 + time_gap²/(2αλ²))^(-α)"
  ),
  stop("Unknown correlation_structure. Choose: ar1, exponential, power_law, matern, rational_quadratic")
)

cat("Using correlation structure:", correlation_structure, "\n")
cat("Description:", correlation_params$description, "\n\n")
...
param_grid <- bind_rows(
  ...
  expand_grid(
    ...
    biomarker_moderation = c(0.25, 0.35, 0.45, 0.55, 0.65),
    biomarker_correlation = c(0.3),
    carryover = c(0, 0.1, 0.2)  # Hendrickson values (weeks)
  ),
  ...
  # Parallel design (no carryover - inherently no within-subject switching)
  expand_grid(
    design = "parallel",
...
# ============================================================================
# CORRELATION STRUCTURE FUNCTIONS
# ============================================================================

# Build correlation matrix based on selected structure
build_correlation_matrix <- function(weeks, structure, params) {
  n <- length(weeks)
  Corr <- matrix(0, n, n)

  if (structure == "ar1") {
    # AR(1): Geometric decay ρ^(time_gap)
    for (i in 1:n) {
      for (j in 1:n) {
        time_lag <- abs(weeks[i] - weeks[j])
        Corr[i, j] <- params$rho^time_lag
      }
    }
  } else if (structure == "exponential") {
    # Exponential/Ornstein-Uhlenbeck: exp(-λ * time_gap)
    for (i in 1:n) {
      for (j in 1:n) {
        time_lag <- abs(weeks[i] - weeks[j])
        Corr[i, j] <- exp(-params$lambda * time_lag)
      }
    }
  } else if (structure == "power_law") {
    # Power Law: 1 / (1 + α * time_gap)^β
    for (i in 1:n) {
      for (j in 1:n) {
        time_lag <- abs(weeks[i] - weeks[j])
        Corr[i, j] <- 1 / (1 + params$alpha * time_lag)^params$beta
      }
    }
  } else if (structure == "matern") {
    # Matérn covariance: requires bessel function
    # Using simplified form: exp(-d/rho) * (1 + d/rho) for ν=1.5
    for (i in 1:n) {
      for (j in 1:n) {
        d <- abs(weeks[i] - weeks[j])
        if (d == 0) {
          Corr[i, j] <- 1.0
        } else {
          # Matérn ν=1.5: (sqrt(3)*d/rho) * K_1(sqrt(3)*d/rho) normalized
          scaled_d <- sqrt(3) * d / params$rho
          Corr[i, j] <- (1 + scaled_d) * exp(-scaled_d)
        }
      }
    }
  } else if (structure == "rational_quadratic") {
    # Rational Quadratic: (1 + time_gap²/(2αλ²))^(-α)
    for (i in 1:n) {
      for (j in 1:n) {
        time_lag <- abs(weeks[i] - weeks[j])
        Corr[i, j] <- (1 + (time_lag^2) / (2 * params$alpha * params$lambda^2))^(-params$alpha)
      }
    }
  } else {
...
# ============================================================================
# BUILD SIGMA WITH GUARANTEED PD
# ============================================================================

build_sigma_guaranteed_pd <- function(weeks, c.bm, params, corr_structure, corr_params) {
  n_tp <- length(weeks)
  ...
  # Get correlation matrices for each component
  Corr_BR <- build_correlation_matrix(weeks, corr_structure, corr_params)
  Corr_ER <- build_correlation_matrix(weeks, corr_structure, corr_params)
  Corr_TR <- build_correlation_matrix(weeks, corr_structure, corr_params)
  ...
  if (min_eig <= 1e-6) {
    stop(sprintf(
      paste(
        ...
        "  1. A bug in the validation, or",
        "  2. Inconsistent covariance matrix construction",
        "",
        "Consider adjusting correlation_structure parameters or measurement schedule.",
        sep = "\n"
      ),
...
# ============================================================================
# PRE-SIMULATION SIGMA VALIDATION (unique structures)
# ============================================================================
...
# All evenly-spaced designs share the same measurement schedule
# Define sigma structures to validate: (design, measurement_weeks, c.bm)
sigma_structures <- list(
  list(design = "ol", weeks = measurement_weeks, c_bm = 0.3),
  list(design = "crossover", weeks = measurement_weeks, c_bm = 0.3),
  list(design = "parallel", weeks = measurement_weeks, c_bm = 0.3),
  list(design = "crossover", weeks = measurement_weeks, c_bm = 0),
  list(design = "parallel", weeks = measurement_weeks, c_bm = 0)
)
...
for (i in seq_along(sigma_structures)) {
  ...
  sigma_result <- tryCatch({
    build_sigma_guaranteed_pd(
      ...
      c.bm = sigma_def$c_bm,
      params = list(),
      corr_structure = correlation_structure,
      corr_params = correlation_params
    )
  }, error = function(e) {
...
if (!all_valid) {
  stop("\n✗ VALIDATION FAILED: One or more sigma matrices are not positive definite.\n",
       "   Check correlation_structure, correlation parameters, or measurement schedule.")
}
...
# Set up logging
log_file <- sprintf("../output/simulation_evenly_spaced_%s_log.txt", correlation_structure)
if (!dir.exists("../output")) dir.create("../output", recursive = TRUE)
sink(log_file, split = TRUE)

cat("Running EVENLY-SPACED designs simulation...\n")
cat(sprintf("Correlation structure: %s\n", correlation_structure))
cat(sprintf("Description: %s\n", correlation_params$description))
...
plot_power_heatmap <- function(data) {
  ...
  p <- ggplot(
    ...
    labs(
      ...
      subtitle = sprintf(
        "N=%d participants, %d iterations per condition | Schedule: weeks %s | Correlation: %s",
        n_participants, n_iterations, paste(measurement_weeks, collapse = ", "), correlation_structure
      ),
...
ggsave(
  file.path(output_dir, sprintf("power_heatmap_evenly_spaced_%s.pdf", correlation_structure)),
  p_heatmap,
  width = 12,
  height = 7
)
...
save(
  results, summary_results, correlation_structure, correlation_params,
  file = file.path(output_dir, sprintf("simulation_evenly_spaced_%s_results.RData", correlation_structure))
)

cat("\nDone! Results saved to", output_dir, "\n")
cat(sprintf("- power_heatmap_evenly_spaced_%s.pdf\n", correlation_structure))
cat(sprintf("- simulation_evenly_spaced_%s_results.RData\n", correlation_structure))
