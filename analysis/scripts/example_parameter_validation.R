...
model_params <- list(
  N = n_participants,
  c.tv = 0.7,          # Time-variant autocorrelation (AR(1) base)
  c.pb = 0.7,          # Pharm biomarker autocorrelation (AR(1) base)
  c.br = 0.7,          # Bio response autocorrelation (AR(1) base)
...
param_grid <- expand_grid(
  ...
  carryover_t1half = c(0, 0.1, 0.2)  # Hendrickson values (weeks)
)
