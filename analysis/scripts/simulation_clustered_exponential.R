...
# Carryover modeled as exponential decay: effect = accum × 0.5^(weeks_off/t1half)
...
# NOTE: carryover now represents t1half (half-life in weeks)
...
cat("Exponential Decay Carryover Model (Alternative 1)\n")
cat(paste0("=" , strrep("=", 79) , "\n\n"))
...
# Set up logging
log_file <- "../output/simulation_clustered_exponential_log.txt"
if (!dir.exists("../output")) dir.create("../output", recursive = TRUE)
sink(log_file, split = TRUE)

cat("Running CLUSTERED designs simulation...\n")
cat("Carryover Model: EXPONENTIAL DECAY (Alternative 1)\n")
...
cat("\n", strrep("=", 50), "\n")
cat("RESULTS - CLUSTERED DESIGNS (EXPONENTIAL DECAY)\n")
cat(strrep("=", 50), "\n\n")
...
plot_power_heatmap <- function(data) {
  ...
  p <- ggplot(
    ...
    labs(
      title = "Statistical Power - Clustered Designs with Exponential Carryover Decay",
      subtitle = sprintf(
        "N=%d participants, %d iterations per condition, half-life in weeks",
...
ggsave(
  file.path(output_dir, sprintf("power_heatmap_clustered_exponential_%s.pdf", timestamp)),
  p_heatmap,
  width = 10,
  height = 7
)
...
save(
  ...
  file = file.path(output_dir, "simulation_clustered_exponential_results.RData")
)

cat("\nDone! Results saved to", output_dir, "\n")
cat(sprintf("- power_heatmap_clustered_exponential_%s.pdf\n", timestamp))
cat("- simulation_clustered_exponential_results.RData\n")
