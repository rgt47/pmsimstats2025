# N-of-1 Trial Simulation - Clustered Measurement Designs
# Hybrid and OL+BDC designs with 8 measurement points including clusters
# FLEXIBLE CORRELATION STRUCTURES: AR1, Exponential, Power Law, Matérn, Rational Quadratic
# These designs have dense measurements during critical transition periods
...
correlation_structure <- "ar1"  # <-- CHANGE THIS TO SELECT STRUCTURE
...
# Parameter grid - Hybrid and OL+BDC designs only
param_grid <- bind_rows(
  ...
  # Type I error condition for OL+BDC
  expand_grid(
    design = "ol_bdc",
...
# Set up logging
log_file <- sprintf("../output/simulation_clustered_%s_log.txt", correlation_structure)
...
cat("Designs: Hybrid, OL+BDC (8 measurement points with clusters)\n")
...
plot_power_heatmap <- function(data) {
  ...
  p <- ggplot(
    ...
    labs(
      title = "Statistical Power - Clustered Designs (8 timepoints)",
      subtitle = sprintf(
        "N=%d participants, %d iterations per condition\nCorrelation: %s",
        n_participants, n_iterations, correlation_structure
      ),
...
ggsave(
  file.path(output_dir, sprintf("power_heatmap_clustered_%s.pdf", correlation_structure)),
  p_heatmap,
  width = 10,
  height = 7
)
...
save(
  ...
  file = file.path(output_dir, sprintf("simulation_clustered_%s_results.RData", correlation_structure))
)

cat("\nDone! Results saved to", output_dir, "\n")
cat(sprintf("- power_heatmap_clustered_%s.pdf\n", correlation_structure))
cat(sprintf("- simulation_clustered_%s_results.RData\n", correlation_structure))
