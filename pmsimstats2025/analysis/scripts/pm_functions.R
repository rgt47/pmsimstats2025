...
#' Calculate carryover-adjusted correlation parameters
#'
#' @param base_correlations Base correlation values (no
#'   carryover)
#' @param carryover_halflife Carryover half-life in weeks
#' @return Adjusted correlation parameters
calculate_carryover_adjusted_correlations <- function(
  ...
  # Range: 0 (no carryover) to 1 (very strong carryover)
  ...
  # More carryover = stronger temporal persistence
  ...
  # BR gets strongest boost
  adjusted_br <- min(0.95, base_autocorr + autocorr_boost * 1.2)

  # Adjust cross-time correlations (increase significantly with
  # carryover)
  cross_time_boost <- 0.4 * carryover_strength
  adjusted_cross_diff <- min(0.8, base_cross_diff +
                                   cross_time_boost)

  # Same-time correlations may decrease slightly (components
  # become more independent)
  cross_same_reduction <- 0.1 * carryover_strength
  adjusted_cross_same <- max(0.05, base_cross_same -
                                    cross_same_reduction)
...
build_sigma_matrix <- function(model_param, resp_param, baseline_param, trial_design,
  ...
  # Check positive definiteness and force if needed
  ...
  if (!is_pd) {
    if (verbose) {
      cat("Making sigma matrix positive definite...\n")
    }
    sigma <- corpcor::make.positive.definite(sigma, tol = 1e-3)
  }
