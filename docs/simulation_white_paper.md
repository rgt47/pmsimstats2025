...
Each simulated dataset is analyzed using a linear mixed-effects model with
continuous-time AR(1) correlation structure (implemented via `nlme::lme` with
`corCAR1`):
...
Components:
- `effective_drug_weeks`: Cumulative treatment exposure with exponential decay
  carryover (EDW)
- `bm_centered`: Biomarker standardized (mean 0, SD 1)
- `EDW:bm_centered`: **Primary endpoint** — biomarker-treatment interaction
- `week`: Time effect (period effect control)
- `corCAR1`: Continuous-time AR(1) correlation matching the DGP
- `(1 | participant_id)`: Random intercept for between-subject variability

**Why `lme` instead of `lmer`**: The `lme4::lmer` package does not support
continuous-time AR(1) correlation. Since our DGP uses time-based AR(1) (where
correlation depends on actual week gaps, not observation indices), we must use
`nlme::lme` with `corCAR1` for proper model-DGP alignment.

**Inference**: The EDW × biomarker interaction is significant if
p < 0.05 (two-sided).
...
| c.br, c.er, c.tr | 0.75 | Within-component autocorrelation (reduced from 0.8 for PD) |
| c.cf1t | 0.12 | Same-time cross-correlation (reduced from 0.2 for PD) |
| c.cfct | 0.05 | Different-time cross-correlation (reduced from 0.1 for PD) |
| c.bm_baseline | 0.25 | Biomarker-baseline correlation (reduced from 0.3 for PD) |
| c.baseline_resp | 0.3 | Baseline-response correlation (reduced from 0.4 for PD) |

**Note**: Correlation parameters were reduced from Hendrickson et al. (2020)
values to ensure positive definiteness of the covariance matrix across all
parameter combinations, particularly with higher biomarker-response correlations.
...
|--------|--------|-------------|
| Design | hybrid, ol_bdc | Trial design structure |
| Biomarker moderation (β_mod) | 0, 0.25, 0.35, 0.45, 0.55, 0.65 | Strength of biomarker × treatment interaction |
| Carryover half-life (t½) | 0, 0.5, 1.0, 2.0 weeks | Exponential decay half-life for treatment persistence |

**Note**: β_mod = 0 is the null condition for Type I error evaluation.
...
2. FOR i IN 1:NROW(param_grid):
   ...
   FOR iter IN 1:n_iterations:
     ...
     # Fit mixed model with lme (nlme) and corCAR1 for AR(1) correlation
     IF params$carryover_halflife > 0:
       model ← LME(response ~ effective_drug_weeks × bm_centered + week,
                   random = ~1|participant_id,
                   correlation = corCAR1(form = ~week|participant_id))
     ELSE:
       model ← LME(response ~ treatment × bm_centered + week,
                   random = ~1|participant_id,
                   correlation = corCAR1(form = ~week|participant_id))

     # Extract interaction test
     coefs ← SUMMARY(model)$tTable
     t_value ← coefs["effective_drug_weeks:bm_centered", "t-value"]
     p_value ← coefs["effective_drug_weeks:bm_centered", "p-value"]

     # Store result
     APPEND to results: {iter, design, biomarker_moderation,
                         carryover_halflife, effect_size, se,
                         t_value, p_value, significant = (p_value < 0.05)}

3. # Summarize results
   summary_results ← results %>%
     GROUP_BY(design, biomarker_moderation, carryover_halflife) %>%
     SUMMARIZE(
...
### 7.5 Effective Drug Weeks Computation

```
ALGORITHM: Compute Effective Drug Weeks (EDW) with Exponential Decay
INPUT: treatment (vector), weeks (measurement times), carryover_halflife (t½)
OUTPUT: effective_drug_weeks (vector)

# Track cumulative weeks on drug and discontinuation events
weeks_on_drug ← CUMSUM(treatment)
tsd ← 0  # time since discontinuation

FOR t IN 1:n_timepoints:
  IF treatment[t] == 1:
    # On treatment: EDW = cumulative weeks on drug
    effective_drug_weeks[t] ← weeks_on_drug[t]
    tsd ← 0
  ELSE:
    # Off treatment: apply exponential decay
    IF just_discontinued[t]:
      weeks_at_discont ← weeks_on_drug[t-1]

    tsd ← weeks[t] - discontinuation_week

    IF carryover_halflife > 0:
      # Exponential decay: retention = (1/2)^(tsd / t½)
      carryover_retention ← (0.5)^(tsd / carryover_halflife)
      effective_drug_weeks[t] ← weeks_at_discont × carryover_retention
    ELSE:
      # No carryover: EDW = 0 when off treatment
      effective_drug_weeks[t] ← 0

RETURN effective_drug_weeks
```

**Example values** for a participant discontinuing after 3 weeks on drug:

| t½ (weeks) | tsd=1 week | tsd=2 weeks | tsd=4 weeks |
|:-----------|:-----------|:------------|:------------|
| 0 | 0 | 0 | 0 |
| 0.5 | 0.75 | 0.19 | 0.01 |
| 1.0 | 1.50 | 0.75 | 0.19 |
...
we enable direct comparison of Hybrid and OL+BDC designs for detecting
treatment effect heterogeneity.

Key findings from this framework:

1. **Methodological alignment**: Our simulation preserves Hendrickson's core
   principles (4-path randomization, carryover in means only) while adapting
   correlation parameters for numerical stability and adding explicit biomarker
   moderation.

2. **AR(1) correlation structure**: Using `nlme::lme` with `corCAR1` ensures
   the analysis model matches the continuous-time AR(1) structure of the DGP,
   which is critical for clustered measurement schedules with irregular spacing.

3. **Variance-power tradeoff**: Longer carryover half-lives reduce the variance
   of effective drug weeks, which increases standard errors and reduces power.
   This is a fundamental statistical property, not model misspecification.
   See `analysis/docs/carryover_variance_whitepaper.tex` for detailed analysis.

4. **Type I error control**: Including β_mod = 0 conditions verifies that tests
   maintain nominal error rates (empirical rates of 3-9% with 100 iterations,
   consistent with 5% nominal level).

5. **Practical guidance**: Results inform design choices for precision medicine
   trials targeting biomarker-treatment interactions, with explicit quantification
   of the power cost of carryover effects.
