...
> **UPDATE (2025-12-17)**: The path descriptions in this document have been
> corrected to match Hendrickson et al. (2020) Figure 2D. The original
> interpretation incorrectly described Paths 1 & 2 as "staying on drug" during
> the entire BD phase. The CORRECT interpretation is that ALL paths discontinue
> during the BD phase - the randomization determines WHEN (week 10 vs week 11),
> not WHETHER. See `RANDOMIZATION_PATHS_DISCUSSION.md` for complete analysis.
...
**Timepoint structure** (matching Hendrickson's `tdNof11` - Figure 2D):
- **Weeks 4, 8**: Open-label phase (OL1, OL2) - all participants ON treatment
- **Weeks 9-12**: Blinded discontinuation (BD1-BD4)
  - Week 9: ALL paths ON (Active)
  - Week 10: RANDOMIZED - Paths 1,2 ON; Paths 3,4 OFF
  - Weeks 11-12: ALL paths OFF (Placebo)
- **Weeks 16, 20**: Crossover phase (COd, COp)
  - Paths 1, 3: Drug first (ON at week 16, OFF at week 20)
  - Paths 2, 4: Placebo first (OFF at week 16, ON at week 20)

**Treatment patterns by path** (CORRECTED - all paths discontinue during BD):
- **Path 1**: 1,1,1,1,0,0,1,0 (Active at BD wk2, CO drug first)
- **Path 2**: 1,1,1,1,0,0,0,1 (Active at BD wk2, CO placebo first)
- **Path 3**: 1,1,1,0,0,0,1,0 (Placebo at BD wk2, CO drug first)
- **Path 4**: 1,1,1,0,0,0,0,1 (Placebo at BD wk2, CO placebo first)
...
- **Path 1 (AB sequence)**: 1,1,1,1,0,0,0,0 (drug first)
- **Path 2 (BA sequence)**: 0,0,0,0,1,1,1,1 (placebo first)
...
```r
# Hybrid Path 1 (participant 1) - CORRECTED per Hendrickson Figure 2D
week  path treatment expectancy period
   4     1         1        1.0      1  # Open-label
   8     1         1        1.0      1  # Open-label
   9     1         1        0.5      2  # BD week 1 (all ON)
  10     1         1        0.5      2  # BD week 2 (randomized - path 1 ON)
  11     1         0        0.5      2  # BD week 3 (all OFF)
  12     1         0        0.5      2  # BD week 4 (all OFF)
  16     1         1        0.5      3  # Crossover (drug first)
  20     1         0        0.5      3  # Crossover (placebo)
```

Pattern matches corrected Hendrickson Path 1: 1,1,1,1,0,0,1,0 ✓
