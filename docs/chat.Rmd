...
- **Classic crossover**: Carryover is typically tested and assumed negligible. If present, it confounds period and treatment effects, potentially invalidating the design. Washout periods are employed to eliminate carryover, and statistical tests for carryover are often underpowered (Jones & Kenward, 2014; Senn, 2002).
...
Y_it = β₀ + β_treat × Treat_it + β_period × Period_t + β_bm × BM_i + β_int × (Treat_it × BM_i) + u_i + ε_it
...
Y_it = β₀ + (β_treat + u_i,treat) × Treat_it + β_period × Period_t + β_bm × BM_it + (β_int + u_i,int) × (Treat_it × BM_it) + u_i + ε_it

- Random intercept (u_i), random slope for treatment (u_i,treat), and optionally random slope for biomarker interaction (u_i,int)
- Individual treatment effect = β_treat + u_i,treat
- Individual biomarker interaction = β_int + u_i,int
- Heterogeneity quantified via Var(u_i,treat) and Var(u_i,int)
- Time-varying biomarkers (BM_it) can link within-person biomarker changes to within-person treatment response

Hybrid Design Mixed Model

Y_it = β₀ + β_treat × Treat_it + β_period × Period_t + β_bm × BM_i + β_int × (Treat_it × BM_i) + β_carry × Carry_it + u_i + ε_it
...
1) Simulate a classic 2-period crossover dataset with biomarker
...
subj_df <- tibble(subject = 1:n_subj,
                  ...
                  bm = rnorm(n_subj, 0, 1))  # standardized biomarker
...
# Treatment effect is modified by biomarker: higher bm = stronger response
beta_period <- 0.5
crossover <- crossover %>%
  mutate(Y = mu + u +
           beta_period * period +
           ...
           beta_bm * bm +
           beta_int * trt_indicator * bm +  # biomarker x treatment
...
m_crossover <- lmer(Y ~ trt_indicator * bm + factor(period) + (1|subject), data = crossover)
summary(m_crossover)
...
# Interpretation: Can estimate population-level effect modification,
# but cannot estimate individual-specific interaction terms

2) Simulate an aggregated N-of-1 dataset with biomarker and carryover

set.seed(2025)
n_subj <- 25
cycles <- 6                # number of periods per subject (e.g., ABABAB)
obs_per_period <- 7        # daily observations per period (1 week)

# True effects
mu <- 20
beta_treat_pop <- -4       # population mean treatment effect
beta_bm <- 2               # biomarker main effect
beta_int_pop <- -1.5       # population biomarker x treatment interaction
beta_carry <- 1.5          # carryover effect

sigma_subj <- 6            # between-subject SD (intercept)
sigma_treat_sd <- 3        # SD of individual treatment effects
sigma_int_sd <- 0.8        # SD of individual biomarker interactions
sigma_resid <- 5
...
subj <- tibble(subject = 1:n_subj,
               ...
               bm_baseline = rnorm(n_subj, 0, 1))  # baseline biomarker
...
# Add time-varying biomarker (baseline + within-person fluctuation)
nof1 <- nof1 %>%
  mutate(bm = bm_baseline + rnorm(n(), 0, 0.3))  # slight daily variation

# Add carryover: effect of prior period's treatment
nof1 <- nof1 %>%
  group_by(subject) %>%
  mutate(prior_trt = lag(trt_indicator, default = 0),
         carryover = if_else(period > 1 & day <= 2, prior_trt, 0)) %>%  # carryover in first 2 days

ungroup()

# Simulate outcomes with all effects
nof1 <- nof1 %>%
  mutate(Y = mu + u +
           (beta_treat_pop + u_trt) * trt_indicator +
           beta_bm * bm +
           (beta_int_pop + u_int) * trt_indicator * bm +  # individual-specific interaction
           beta_carry * carryover +
           rnorm(n(), 0, sigma_resid))

# Inspect: now have dense data
nof1 %>% group_by(subject) %>% summarise(n_obs = n(), n_periods = n_distinct(period)) %>% head()

Fit aggregated N-of-1 hierarchical mixed model with biomarker interaction

# Full model: random intercept + random slope for treatment + biomarker interaction + carryover
# Note: Random slope for biomarker interaction may cause convergence issues; start simpler
m_nof1 <- lmer(Y ~ trt_indicator * bm + carryover + (1 + trt_indicator | subject),
               data = nof1, REML = TRUE)
summary(m_nof1)
...
subj_trt_effects <- ranef(m_nof1)$subject[["trt_indicator"]] + fixef(m_nof1)["trt_indicator"]
subj_trt_effects

# Advanced: Add random slope for biomarker interaction (may need more data or regularization)
# This estimates individual-specific biomarker x treatment interactions
# m_nof1_full <- lmer(Y ~ trt_indicator * bm + carryover +
#                     (1 + trt_indicator + trt_indicator:bm | subject),
#                     data = nof1, REML = TRUE,
#                     control = lmerControl(optimizer = "bobyqa"))

# Compare to Bayesian hierarchical model with brms (optional)
# Bayesian approach handles complex random effect structures better
# brm_nof1 <- brm(Y ~ trt_indicator * bm + carryover + (1 + trt_indicator | subject),
#                 data = nof1, chains = 2, iter = 2000)
# summary(brm_nof1)
...
ggplot(subj_effects, aes(x = subj_trt)) +
 ...
 geom_vline(xintercept = fixef(m_nof1)["trt_indicator"], linetype = "dashed", color = "red") +
 labs(title = "Distribution of individual treatment effects (BLUPs)",
...
# Show how treatment effect varies with biomarker at population level
bm_range <- seq(-2, 2, by = 0.1)
interaction_df <- tibble(
 bm = bm_range,
 trt_effect = fixef(m_nof1)["trt_indicator"] + fixef(m_nof1)["trt_indicator:bm"] * bm_range
)

ggplot(interaction_df, aes(x = bm, y = trt_effect)) +
 geom_line(size = 1.2, color = "darkgreen") +
 geom_hline(yintercept = 0, linetype = "dotted") +
 labs(title = "Biomarker x Treatment Interaction",
      subtitle = "Treatment effect as function of biomarker level",
      x = "Biomarker (standardized)", y = "Treatment effect (CAPS change)")

# Observed trajectories for a few subjects, showing daily measurements
sample_subj <- sample(unique(nof1$subject), 6)
nof1 %>% filter(subject %in% sample_subj) %>%
 mutate(time = (period - 1) * obs_per_period + day,
        trt = if_else(trt_indicator == 1, "Active", "Placebo")) %>%
 ggplot(aes(x = time, y = Y, color = trt)) +
 geom_line(alpha = 0.7) + geom_point(size = 0.8) +
 facet_wrap(~ subject, scales = "free_y") +
 labs(title = "Individual patient trajectories",
      x = "Day", y = "CAPS score", color = "Treatment")
